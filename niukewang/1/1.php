...
echo '<br>';
function test3($str){
    if(!is_string($str)) return '输入的不是字符串';
    $len = strlen($str);
    for($i = 1;$i <= $len; $i++){
        $str[$i] = substr($str,$i-1,$i);
        if($i > 1){
            if($str[$i] == $str[$i-1]);
        }
